...
- Profile: custom (1080×1920, 24 FPS, 16 秒，不循環、無音訊)。
- Act 1 (0–4 秒): JSON fragments 收斂成 `lottie-maker`，建立「不只讓 JSON 動起來」的問題。
- Act 2 (4–8 秒): 16:9、9:16、1:1 使用不同幾何，表達內容重組而非等比縮放。
- Act 3 (8–12 秒): Create → Inspect → Render A/B → SHA-256 match；unsafe branch 被擋下。
- Act 4 (12–16 秒): 四個互相咬合、反向旋轉的齒輪表達 Create、Revise、Diagnose、Verify 循環改善。
...
- QA: 檢查四個 checkpoint 的閱讀順序、safe area、文字 fit、卡片 padding、齒輪間距、轉向、轉場、完整 MP4 與 deterministic hashes。
