...
- Act 3 (8–12 秒): Create → Inspect → Render A/B → SHA-256 match；連接線只存在於卡片之間，不穿過標籤；unsafe branch 被擋下。
- Act 4 (12–16 秒): 四個等齒數齒輪依實際多邊形齒廓包絡設定 251 px 工作中心距，先依序進場，再從同一 frame 以 15° 半齒距相位差、棋盤格反向等速旋轉，表達 Create、Revise、Diagnose、Verify 循環改善。
...
- QA: 檢查四個 checkpoint 的閱讀順序、safe area、文字 fit、卡片 padding、流程線不穿卡片、齒輪 251 px 工作中心距、15° 半齒距相位、等速反向、轉場、完整 MP4 與 deterministic hashes。
